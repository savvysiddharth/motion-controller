const ctrack = new p5(
  (p) => {
    ...
    p.setup = () => {
      ...
      trackCanvas = p.createCanvas(500, 500);
      trackCanvas.hide();
    };
    ...
    p.draw = () => {
      ...
      p.x = tracked.x;
      p.y = tracked.y;
      p.count = tracked.count;

      if(showingNow) {
        p.background(0);
        p.translate(500, 0);
        p.scale(-1,1);
        p.image(cam,0,0, 500,500);
        p.translate(0, 0);
        p.scale(1,1);
        p.fill(0,0,250, 170);
    ...
    function doTrack() {
      ...
      targetX = p.map(targetX, 0, 500, p.displayWidth, 0);
      targetY = p.map(targetY, 0, 500, 0, p.displayHeight);
      return { x: targetX, y: targetY, count: count };
    }
